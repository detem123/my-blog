...
export default function WritePage() {
  ...
  useEffect(() => {
    ...
    if (editId) {
      articleAPI.getBySlug(editId).then(res => {
        ...
        setCategoryName(a.categoryName || ''); setTags(a.tags || []);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-6">{editId ? '编辑文章' : '写文章'}</h1>
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 space-y-5">
        ...
        <div className="flex gap-3 items-center flex-wrap">
          ...
          <div className="flex-1 flex gap-2 flex-wrap items-center">
            {tags.map(tag => (
              <span key={tag} className="inline-flex items-center gap-1 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 px-2.5 py-1 rounded-full text-xs font-medium">
                {tag}
            ...
            <input type="text" value={tagInput} onChange={e => setTagInput(e.target.value)}
              ...
              placeholder="+ 标签" className="w-20 px-2 py-1 border-0 text-xs focus:outline-none bg-transparent text-slate-600 dark:text-slate-400" />
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
          <button onClick={() => handleSave(false)} disabled={saving}
            className="flex items-center gap-2 px-5 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-600 dark:text-slate-400 text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition">
            <HiSave /> 保存草稿
          </button>
          <button onClick={() => handleSave(true)} disabled={saving}
            className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-700 disabled:opacity-50 transition shadow-md shadow-indigo-200 dark:shadow-indigo-900">
